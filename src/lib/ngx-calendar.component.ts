  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => NgxCalendarComponent),
      multi: true,
    },
  ],
  standalone: true,
...
export class NgxCalendarComponent implements ControlValueAccessor {
  private readonly datePickerCalendarService = inject(NgxDatePickerCalendarService);
  ...
  monthName = computed(() =>
    this.datePickerCalendarService.getMonthName(this.currentMonth())
  );
  ...
  private getDefaultWeekdayNames(): string[] {
    const names = this.datePickerCalendarService.getWeekdayNames();
    const startOfWeek = this.startOfWeek();
    return startOfWeek === 0 ? names : [...names.slice(1), names[0]];
  }

  monthNames = computed(() => this.datePickerCalendarService.getMonthNames());
  ...
  calendarDays = computed(() => {
    ...
    return this.datePickerCalendarService.generateCalendarDays(
      this.currentYear(),
      this.currentMonth(),
      disabledDates,
      selectedDates,
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (preSelectedDate) {
        ...
        this.selectedDates.set(this.datePickerCalendarService.getDateRange(preSelectedRange.start, preSelectedRange.end));
  ...
  onDayClick(day: CalendarDay): void {
    ...
    if (mode === 'single') {
      ...
      const dates = [...this.selectedDates()];
      const index = dates.findIndex((d) => this.datePickerCalendarService.isSameDay(d, day.date));
      ...
      if (!start) {
        ...
        const range = this.datePickerCalendarService.getDateRange(
          this.rangeStart()!,
          this.rangeEnd()!
        );
   ...
   */
  previousMonth(): void {
    this.currentDate.set(this.datePickerCalendarService.subtractMonths(this.currentDate(), 1));
  }
   ...
   */
  nextMonth(): void {
    this.currentDate.set(this.datePickerCalendarService.addMonths(this.currentDate(), 1));
  }
  ...
  getDayClass(day: CalendarDay): string {
    ...
    if (day.isSelected && this.selectionMode() === 'range') {
      const start = this.rangeStart();
      const end = this.rangeEnd();
      if (start && this.datePickerCalendarService.isSameDay(day.date, start)) {
        classes.push('range-start');
      }
      if (end && this.datePickerCalendarService.isSameDay(day.date, end)) {
